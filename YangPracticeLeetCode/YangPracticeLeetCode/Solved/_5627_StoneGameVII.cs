...
namespace YangPracticeLeetCode.Solved
{
	class _5627_StoneGameVII
	{
		...
		{
			Solution_V2_Cache_DP3 s = new Solution_V2_Cache_DP3();
		...
		///
		/// /Time Complexity : \mathcal{O}(2^{n^{2}})O(2 
		///	n 
		///	2

		///	). For an array of length nn, there are roughly { n^{ 2} }
		///n 
		///2
		///possible subarrays.

		///We fill the array prefixSum of size n by iterating n times.The time complexity would be \mathcal{ O}(n))O(n)).

		///For each subarray there are 22 choices, either remove the first element or remove the last element.Thus the time complexity of recursive function would be \mathcal{ O}(2^{n^{2}})O(2 
		///n
		///2

		///).

		///This would give us total time complexity as \mathcal{O
		///}(n)) + \mathcal{O}(2^{n^{2}}) = \mathcal{O}(2^{n^{2}})O(n))+O(2 
		///n
		///2
